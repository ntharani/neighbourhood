...
var PlaceModel = function () {
  function PlaceModel(place) {
    ...
    this.mapMarker.addListener('click', function () {
      vm.hideWindows();
      _this.mapMarker.setAnimation(google.maps.Animation.BOUNCE);
      setTimeout(function () {
        _this.mapMarker.setAnimation(null);
      }, 1500);
      map.panTo({ lat: _this.lat, lng: _this.lng });
      _this.infoWindow.open(map, _this.mapMarker);
    });
  }
...
var PlaceViewModel = function () {
  function PlaceViewModel() {
    ...
    _classCallCheck(this, PlaceViewModel);
    ...
    this.filterResults = ko.computed(function () {
      ...
      var re = new RegExp(_this2.filter(), 'i');

      // Iterate over all place objects, searching for a matching name
      _this2.placeList().forEach(function (place) {
    ...
    getfsdata(function (result) {
      ...
      result.forEach(function (obj) {
        _this2.placeList.push(new PlaceModel(obj));
      });
    });
  }

  _createClass(PlaceViewModel, [{
    key: 'hideWindows',
    value: function hideWindows() {
      console.log("hiding all Infowindows");
      // Is there a more efficient way that continually iterating over this each time?
      this.placeList().forEach(function (obj) {
        obj.hideInfoWindow();
      });
    }
